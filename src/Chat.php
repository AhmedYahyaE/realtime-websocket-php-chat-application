...
class Chat implements MessageComponentInterface { // The 'Chat' class is our custom WebSocket handler class that implements the 'MessageComponentInterface' of the Ratchet library
    ...
    public function onOpen(ConnectionInterface $conn) { // Note:    $conn->resourceId    is a unique identifier generated by the WebSocket Server for every connected client (user) (is also called WebSocket Session ID)
        ...
        // Disply the Online/Offline Status of all users/clients in case of 'One-to-One/Private' Chat ONLY, not 'Group' Chat (This script is repeated inside the onClose() method of this class for the Online/Offline Status too (Offline Status))
        ...
        if (isset($queryarray['token'])) // If the chat is 'One-to-One' or 'Private' Chat (not 'Group' Chat) i.e. The JavaScript WebSocket Constructor function URL contains a 'token' as a query string parameter e.g.    var conn = new WebSocket('ws://localhost:8080?token=<?php echo $token; >');    (which comes from the JavaScript in privatechat.php)
        {
            ...
            $data['status_type']    = 'Online'; // Append 'status_type'    to the $data array
            $data['user_id_status'] = $user_id; // Append 'user_id_status' to the $data array
    ...
    public function onMessage(ConnectionInterface $from, $msg) { // $from and $msg (they are the 'userId' key and 'msg' key of the 'data' JavaScript object in chatroom.php) inside the JavaScript 'data' variable in chatroom.php) are sent by JavaScript (under the "Handling Chat HTML Form Submission" Section in chatroom.php) using the    conn.send()    function in chatroom.php    // This method is triggered when a chat message is sent from the client side chatroom.php (via JavaScript in the <script> tag), then it takes this message and, in turn, it sends it to all other clients (receivers), including the sender themselves    // $msg    is the JSON chat message sent from the client side WebSocket ($msg is the chat message that is sent when the Chat HTML Form is submitted using JavaScript in chatroom.php. Check the <script> HTML tag in the same file (Under the "Handling Chat HTML Form Submission" Section in chatroom.php)), and it's passed on (sent again) to all other connected users' client side (i.e. Browser/JavaScript) (it's sent again to all users's client side (Browser/JavaScript) connected to the chat WebSocket)
    ...
    public function onClose(ConnectionInterface $conn) {
        ...
        // Disply the Online/Offline Status of all users/clients in case of 'One-to-One/Private' Chat ONLY, not 'Group' Chat (This script is repeated inside the onOpen() method of this class for the Online/Offline Status too (Online Status))
